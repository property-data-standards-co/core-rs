...
pub struct VerifyVcOptions<'a> {
    ...
    /// Pre-fetched status list encoded bitstring. If None and credential has status, check is skipped with warning.
    ...
    if let Some(registry) = options.tir_registry {
        ...
        if !tir_result.trusted {
            result.warnings.push(format!(
                "TIR: issuer not fully authorised. Uncovered: {:?}",
    ...
    if let Some(status) = &options.vc.credential_status {
        if let Some(bitstring_b64) = options.status_list_bitstring {
            match check_credential_status(status, bitstring_b64) {
                ...
                Err(e) => {
                    result.warnings.push(format!("Status check error: {e}"));
                }
            }
        } else {
            result
                .warnings
                .push("Credential has status but no status list provided for verification".into());
        }
    }
...
async fn verify_signature(
    ...
    let proof = vc.proof.as_ref().ok_or_else(|| {
        PdtfError::VerificationError("No proof to verify".into())
    })?;
    ...
    let vm = vms
        ...
        .ok_or_else(|| {
            PdtfError::VerificationError(
                "Could not find matching verification method".into(),
            )
        })?;

    let multibase = vm.public_key_multibase.as_ref().ok_or_else(|| {
        PdtfError::VerificationError(
            "Verification method missing publicKeyMultibase".into(),
        )
    })?;
...
mod tests {
    ...
    use crate::status::bitstring::{create_status_list, encode_status_list, set_bit, MIN_BITSTRING_SIZE};
    ...
    async fn make_signed_vc() -> (VerifiableCredential, MemoryKeyProvider) {
        ...
        let signer = VcSigner::from_key_id(&provider, "test-key")
            .await
            .unwrap();
    ...
    async fn test_verify_revoked_credential() {
        ...
        let signer = VcSigner::from_key_id(&provider, "rev-key")
            .await
            .unwrap();
